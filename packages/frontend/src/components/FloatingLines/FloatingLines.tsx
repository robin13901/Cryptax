...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    if (linesGradient && linesGradient.length > 0) {
      ...
      const gradientArr = uniforms.lineGradient.value as Vector3[];
      stops.forEach((hex, i) => {
        const color = hexToVec3(hex);
        gradientArr[i].set(color.x, color.y, color.z);
      });
    }
    ...
    const renderLoop = () => {
      ...
      if (interactive) {
        ...
        (uniforms.bendInfluence as { value: number }).value = currentInfluenceRef.current;
      }
